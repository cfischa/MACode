...
class DataFrameParser(object):
            ...
            if datatype in ['O', '<U32']:
                ...
                if len(repeated_entries) == 1:
                    ...
                    self.new_dataframe[new_column_name] = \
                        self.new_dataframe[column].apply(
    ...
    def invert_fit(self, encoded_table):
        ...
        for column, encoder in self.encoders.items():
            if column in self.numerical_columns:
                ...
                decoded_table[column] = LabelEncoder().fit_invert(self.new_dataframe[column].astype(int),
                                                                  encoded_table[column])

            else:
                decoded_table[column] = LabelEncoder().fit_invert(self.new_dataframe[column].astype(str),
                                                                  encoded_table[column])
    ...
    def datatype_info(self):
        return {'n_bins': self.n_bins, 'n_cats': self.n_cats, 'n_nums': self.n_nums, 'cards': self._cards}
...
def convert_to_table(org_df, synth_data, threshold):
    ...
    # **Changed:** Ensure the data is moved to CPU before reshaping
