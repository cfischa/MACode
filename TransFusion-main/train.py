...
def visualize(ori_data, fake_data, dataset_name, seq_len, save_path, epoch, writer):
    ...
    # Highlighted change: Ensuring the index is within bounds
    randn_num[0] = randn_num[0] % real_sample.shape[0]  # Ensure index is valid

    axs[1, 0].plot(real_sample[randn_num[0], :, :])  # Use the adjusted index

    axs[1, 0].set_title('Original Data')

    axs[1, 1].plot(fake_sample[randn_num[0], :, :])  # Use the same adjusted index
